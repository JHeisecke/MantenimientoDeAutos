...
#_______________________________Menu___Solicitudes_____________________________
	def add_solicitud(self):
		...
		resp = input("Necesita algun repuesto?",("s","n"))
		while resp == "s":
			print("\n---Repuesto---")
			datosR = Repuesto.prompt_init()
			repuesto = Repuesto(**datosR)
			solicitud.repuestos.append(repuesto)
			print()
			resp = input_opcion("Desea ingresar otro repuesto?", ("s", "n"))
		try:
	...
	def baja_solicitud(self):
		...
		if bd.solicitudes:
			pos = input_entero("Ingrese posicion de solicitud a dar baja")
			if pos is not None:
				#try:
					#bd.solicitudes[pos - 1].mostrar()
				resp = input_opcion("Proseguir", ("s", "n"))
				if resp == "s":
	...
	def calc(self, dato):
		...
		print(("Cliente: {}".format(dato.cliente)))
		print(("Empleado: {}".format(dato.asesor)))
		money = 0
		if dato.vehiculo:
			money += costo_general
			if dato.repuestos:
				for rep in dato.repuestos:
					money += dato.repuesto.costo
...
#_________________________________LISTADO_________________________________
	...
	def listar_solicitudes(self, p=True):
		...
		if bd.solicitudes:
			...
			for val in bd.solicitudes:
				...
				print()
				print("\t---Detalle Repuestos---")
				if val.repuestos:
					for rep in val.repuestos:
						print("\ttipo: {}".format(rep.tipo))
						print("\tmarca: {}".format(rep.marca))
						print("\tcosto: {}".format(rep.costo))
					print()
				numero += 1
			if p:
				input("Presione enter para volver al menu...")
	...
	def listar_solicitudes_baja(self, p=True):
		"""Permite listar los datos contenidos en los distintos vectores
		   El valor p = False sirve para imprimir sin pausas"""
		numero = 1
		if bd.solicitudes_baja:
			print()
			for val in bd.solicitudes_baja:
				print("-----------------==Solicitudes Atendidas==-----------------")
				print("-----------------==========={}===========-----------------".format(numero))
				print(("Fecha: {}".format(val.fecha)))
				print(("Cliente: {}".format(val.cliente)))
				print(("Asesor: {}".format(val.asesor)))
...
#___________________________________MENUS______________________________________
	...
	opciones[3] = {"clave": "Menu de Solicitudes", "funcion": menu_solicitudes}
	...
	o_solicitudes[3] = {"clave": "Dar de Baja Solicitudes", "funcion": baja_solicitud}
	o_solicitudes[4] = {"clave": "Listar Solicitudes Pendientes", "funcion": listar_solicitudes}
	o_solicitudes[5] = {"clave": "Listar Solicitudes Atendidas", "funcion": listar_solicitudes_baja}
	o_solicitudes[6] = {"clave": "Salir", "funcion": fin}
#______________________________________________________________________________
#______________________________________________________________________________
#______________________________________________________________________________
